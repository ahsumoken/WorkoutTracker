const SESSION_TYPES = {
  'compounds-a': {
    name: 'Compounds A', tag: 'GYM A', color: '#3b82f6', type: 'gym',
    exercises: [
  ...
  'compounds-b': {
    name: 'Compounds B', tag: 'GYM B', color: '#06b6d4', type: 'gym',
    exercises: [
  ...
  'kettlebell': {
    name: 'Kettlebell Circuit A', tag: 'CIRCUIT A', color: '#f97316', type: 'circuit',
    rounds: 3, workSec: 45, restSec: 15, roundRestSec: 90,
    exercises: [
  ...
  'kettlebell-b': {
    name: 'Kettlebell Circuit B', tag: 'CIRCUIT B', color: '#f97316', type: 'circuit',
    rounds: 3, workSec: 45, restSec: 15, roundRestSec: 90,
    exercises: [
  ...
  'alan-a': {
    name: 'Alan Hanik A', tag: 'ALAN A', color: '#a855f7', type: 'circuit',
    rounds: 3, workSec: 45, restSec: 15, roundRestSec: 90,
    exercises: [
  ...
  'alan-b': {
    name: 'Alan Hanik B', tag: 'ALAN B', color: '#22c55e', type: 'circuit',
    rounds: 3, workSec: 45, restSec: 15, roundRestSec: 90,
    exercises: [
  ...
  'spartan-50': {
    name: 'The Spartan 50', tag: 'SPARTAN 50', color: '#ef4444', type: 'circuit',
    rounds: 1, workSec: 0, restSec: 0, roundRestSec: 0,
    exercises: [
      { id: 'spartan_rep', name: 'Burpee (3 Push-ups) + 3 Jump Squats', defaultWeight: 'Gewichtsvest' }
    ],
    finisher: null
  },
  'bodyweight-murph': {
    name: 'Bodyweight Murph Variant', tag: 'MURPH', color: '#6366f1', type: 'circuit',
    rounds: 10, workSec: 0, restSec: 0, roundRestSec: 0,
    exercises: [
  ...
  'kb-rope-amrap': {
    name: 'KB & Springtouw AMRAP', tag: 'AMRAP', color: '#f43f5e', type: 'circuit',
    rounds: 1, workSec: 1200, restSec: 0, roundRestSec: 0,
    exercises: [
      { id: 'am_rope',  name: '100x Double Unders / 200x Normaal', defaultWeight: 'Springtouw' },
      { id: 'am_clean', name: '15x Double KB Cleans',            defaultWeight: '2×16kg' },
      { id: 'am_squat', name: '15x Front Squats (Rack)',          defaultWeight: '2×16kg' },
      { id: 'am_press', name: '10x Clean & Presses',              defaultWeight: '2×16kg' }
    ],
    finisher: null
  },
  'ring-quest': {
    name: 'The Ring Quest', tag: 'RING QUEST', color: '#0ea5e9', type: 'circuit',
    rounds: 1, workSec: 0, restSec: 0, roundRestSec: 0,
    exercises: [
  ...
  'snacks': {
    name: 'Kettlebell Snacks', tag: 'SNACK', color: '#eab308', type: 'snacks',
    options: [
      { id: 'power', name: 'Snack 1 — Power', duration: '5 min', protocol: '5 ronden · geen rust', rounds: 5, restSec: 0, exercises: [ 'KB Swing 20kg × 10', 'Burpees × 5' ] },
      { id: 'upper', name: 'Snack 2 — Upper Body', duration: '7 min', protocol: '3 ronden · 30sec rust', rounds: 3, restSec: 30, exercises: [ 'Renegade Row 2×16kg × 10', 'Clean & Press 2×16kg × 10', 'Bench Dips × 10' ] },
      { id: 'lower', name: 'Snack 3 — Lower Body', duration: '7 min', protocol: '3 ronden · 30sec rust', rounds: 3, restSec: 30, exercises: [ 'KB Swing 20kg × 15', 'Goblet Squat 20kg × 10', 'RDL 2×20kg × 10' ] },
      { id: 'core', name: 'Snack 4 — Core & Pull', duration: '5 min', protocol: '3 ronden · 30sec rust', rounds: 3, restSec: 30, exercises: [ 'Negatieve pull-ups × 10', 'Renegade Row 2×16kg × 10' ] },
      { id: 'fullbody', name: 'Snack 5 — Full Body Express', duration: '10 min', protocol: 'AMRAP', rounds: null, restSec: 0, exercises: [ 'KB Swing 20kg × 10', 'Goblet Squat 20kg × 5', 'Burpees × 5', 'Clean & Press 2×16kg × 5' ] }
    ]
  }
};
...
const DB = {
  ...
  getSessions() { try { return JSON.parse(localStorage.getItem(this.KEY) || '[]'); } catch { return []; } },
  saveSession(s) { const all = this.getSessions(); all.unshift(s); localStorage.setItem(this.KEY, JSON.stringify(all)); },
  getLastByType(type) { return this.getSessions().find(s => s.type === type) || null; },
  getLastSnack(snackId) { return this.getSessions().find(s => s.type === 'snacks' && s.snackId === snackId) || null; },
